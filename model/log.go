...
// GetLogUsageBreakdown 聚合消费日志（type=LogTypeConsume）的模型分布与分组使用分布。
// 仅执行两条 GROUP BY 查询，供数据看板分布饼图使用；不再计算 summary（quota/count/token/avg_use_time），
// 因为这些累计指标已由看板顶部的统计卡片通过 /api/data 的 data 数组单独计算得出。
func GetLogUsageBreakdown(startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string) (modelBreakdown []UsageBreakdownItem, groupBreakdown []UsageBreakdownItem, err error) {
	base := LOG_DB.Table("logs").Where("type = ?", LogTypeConsume)
	if base, err = applyExplicitLogTextFilter(base, "model_name", modelName); err != nil {
		return modelBreakdown, groupBreakdown, err
	}
	...
	err = base.Session(&gorm.Session{}).
		...
		Group("model_name").
		Order("quota DESC").
		Scan(&modelBreakdown).Error
	if err != nil {
		common.SysError("failed to query model usage breakdown: " + err.Error())
		return modelBreakdown, groupBreakdown, errors.New("查询模型分布失败")
	}
	...
	if err != nil {
		...
		return modelBreakdown, groupBreakdown, errors.New("查询分组分布失败")
	}

	return modelBreakdown, groupBreakdown, nil
}
